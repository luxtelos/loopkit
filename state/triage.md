...
| Criterion 33 cites a prompt-bytes-per-tick ceiling that does not exist anywhere in the repo — an unpinned proxy presented as a check | PR #8 review §phantom-proxy | high |  | fixing |
...
| loopkit-init.sh re-adds the .loopkit/config.env ignore line even when a project deliberately removed it — no marker distinguishes 'never had it' from 'removed on purpose' | init run 2026-09-07 §gitignore | medium |  | fixing |
| check-citations is vacuous AS CONFIGURED, not unfailable: it fails rc=1 on a bogus citation, but scans 0 citations with pins:[] and specs/ unscanned. Scope it at specs/ and pin real references | PR #15 review §empty-citation-check | medium |  | fixing |
| CI runner has no model checkers, so the runtime model's invariants are unverified there; the pin now skips loudly. Install the engines in the workflow or accept macOS-only proof and say so in the README | CI 2026-09-07 §model-engines | medium |  | fixing |
| targets is asserted in fixtures 01/04/05 but defined nowhere in the spec; fixture-derivable.py's targets_of() encodes one reading of an undefined noun, so the derivability pin reads as covering it and does not. README rule 6 is now false | PR #15 review §targets-undefined | high |  | fixing |
| result.status values are never enumerated in the runtime spec | PR #15 review §status-unenumerated | medium |  | fixing |
...
| Release 0.2.2: governance guard fixes reach installed copies only after the manifest bump merges | release §0.2.2 | high |  | pr-open |
