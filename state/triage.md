...
| check-citations is vacuous AS CONFIGURED, not unfailable: it fails rc=1 on a bogus citation, but scans 0 citations with pins:[] and specs/ unscanned. Scope it at specs/ and pin real references | PR #15 review §empty-citation-check | medium |  | new |
...
| targets is asserted in fixtures 01/04/05 but defined nowhere in the spec; fixture-derivable.py's targets_of() encodes one reading of an undefined noun, so the derivability pin reads as covering it and does not. README rule 6 is now false | PR #15 review §targets-undefined | high |  | new |
| result.status values are never enumerated in the runtime spec | PR #15 review §status-unenumerated | medium |  | new |
