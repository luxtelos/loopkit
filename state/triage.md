...
|---|---|---|---|---|
| Judge runner: judge.py runs a pairwise, position-swapped LLM judge via claude -p (references/judge.md made executable) | plan §0.3 judge-runner | medium | specs/pairwise-judge-verdicts.md | pr-open |
| PreToolUse updatedInput offload: rewrite a large Bash result to .loopkit/scratch and return the path | plan §0.3 updatedInput-offload | medium | specs/bash-output-offload.md | pr-open |
...
| M1 runtime spec: specs/loopkit-runtime.md (EARS), ADR-0001..0004, docs/DDD-ERD.md, spec/fixtures, loopkit-runtime.model.fizz | docs/research/runtime-plan.md §M1 | high |  | spec-draft |
