...
| CREDENTIAL LEAK: S3Store.__init__ sets self.host = parts.netloc, which carries user:password@ — so the password reaches every error message, the outgoing Host header, and the SigV4 canonical request, meaning it is signed into the request | PR #27 review §leak-s3-netloc | critical |  | fixing |
