...
| CREDENTIAL LEAK: S3Store.__init__ sets self.host = parts.netloc, which carries user:password@ — unscrubbed it reaches the outgoing Host header (store.py:579) and the SigV4 canonical request that is then signed (store.py:614). The two transport error paths at :633 and :637 DO pass through _scrub, so the leak is narrower than every error message and still a credential signed into the request | PR #27 review §leak-s3-netloc | critical |  | fixing |
