...
#
# usage: bash "${CLAUDE_PLUGIN_ROOT}/scripts/remote-gate.sh" [ref] [suite-command]
#
# It lives inside the plugin, not in this repository's tools/, because that is
# the only place a consuming project can reach it: `tools/` is not part of what
# ships and init does not copy it. The agent files that send readers here use
# ${CLAUDE_PLUGIN_ROOT}, the same idiom as every other shipped instruction.
#
