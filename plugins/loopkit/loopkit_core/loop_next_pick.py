...
def load_scopes(root: Path) -> dict[str, list[str]]:
    """Lane vocabulary from <project>/.loopkit/scopes.json.

    Byte-for-byte the body of `loop-scan.py`'s `load_scopes`, not a call to it.
    Before M2 this module reached into `scripts/loop-scan.py` by path and fell
    back to a slightly DIFFERENT inline parse when that failed — the fallback
    dropped `if str(t).strip()`, so a scopes file with an empty term resolved
    two ways depending on which branch ran. `loopkit_core` may not import a
    script that shells out to `gh`, so the shared body lives here and
    `loop-scan.py` keeps its own copy; a scopes file now resolves one way.
    """
    try:
...
def scope_terms(scope: str) -> list[str]:
    """A lane name from <project>/.loopkit/scopes.json, or a comma list of
    terms. Empty means unscoped."""
    if not scope:
        return []
    table = load_scopes(project_root())
