...
| The owner is asked something they already ruled on                    | Escalate to be safe                                            | `knowledge`    | `memory.py recall` first — it searches the notes and the bundle. If the ruling exists, record or link it and close; re-asking a settled question teaches the owner that escalations can be ignored.                        |
| The same trap bites a second time                                     | Fix it again, faster                                           | `knowledge`    | Record the Trap in the bundle with the lane tag; the tick doctrine injects lane traps before the work starts. Recurrence within 30 days is the metric.                                                                    |
